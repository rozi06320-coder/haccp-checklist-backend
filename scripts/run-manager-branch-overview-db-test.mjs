#!/usr/bin/env node
/**
 * Isolated test runner for Manager Daily Inventory Branch Overview.
 *
...
const DISPOSABLE_DB_NAME = 'manager_tracking_test_disposable';
...
function runSqlInDisposableDb(sqlFilePath) {
  ...
  if (dockerContainer) {
    ...
    if (res.status !== 0) {
      console.error(`\x1b[31mError applying ${sqlFilePath} to TEST_DATABASE_URL:\x1b[0m\n${res.stderr || res.stdout}`);
      process.exit(1);
    }
    return res.stdout;
  } else {
...
console.log('\x1b[34m  MANAGER BRANCH OVERVIEW DISPOSABLE TEST RUNNER\x1b[0m');
console.log('\x1b[34m====================================================\x1b[0m');
...
// Step 2: Ensure organization_memberships.active exists
runSqlInDisposableDb('supabase/migrations/20260808020000_internal_admin_organization_manager_provisioning.sql');

// Step 3: Ensure private.actor_manages_active_organization exists
runSqlInDisposableDb('supabase/migrations/20260812041700_lifecycle_authorization_hardening.sql');

// Step 4: Apply Phase 3A
runSqlInDisposableDb('supabase/migrations/20260909100000_product_inventory_recipe_catalogs.sql');

// Step 5: Apply Phase 3B1
runSqlInDisposableDb('supabase/migrations/20260910100000_product_sales_snapshots_phase3b1.sql');

// Step 6: Apply Phase 3B2 Daily Waste
runSqlInDisposableDb('supabase/migrations/20260910110000_daily_waste_phase3b2.sql');

// Step 7: Apply Daily Inventory Persistence
runSqlInDisposableDb('supabase/migrations/20260912100000_daily_inventory_persistence.sql');

// Step 8: Apply Manager Tracking Read Model
runSqlInDisposableDb('supabase/migrations/20260915100000_manager_daily_inventory_tracking_read_model.sql');

// Step 9: Apply Manager Daily Inventory Branch Overview (under test)
runSqlInDisposableDb('supabase/migrations/20260915110000_manager_daily_inventory_branch_overview.sql');

// Step 10: Run pgTAP test suite
console.log('\x1b[36m--> Running pgTAP test suite: supabase/tests/database/manager_daily_inventory_branch_overview.test.sql...\x1b[0m');
const tapOutput = runSqlInDisposableDb('supabase/tests/database/manager_daily_inventory_branch_overview.test.sql');
