...
insert into public.purchasing_memberships(organization_id,user_id,active,created_by,updated_by) values
 ('24000000-0000-4000-8000-000000000001','14000000-0000-4000-8000-000000000008',true,'14000000-0000-4000-8000-000000000004','14000000-0000-4000-8000-000000000004'),
 ('24000000-0000-4000-8000-000000000001','14000000-0000-4000-8000-000000000009',false,'14000000-0000-4000-8000-000000000004','14000000-0000-4000-8000-000000000004');
...
select throws_ok($$select public.finalize_password_change('14000000-0000-4000-8000-000000000009')$$,'42501','password change finalization denied','inactive purchasing member rejected');
...
select lives_ok($$select public.finalize_password_change('14000000-0000-4000-8000-000000000008')$$,'purchasing member succeeds');
reset role;
...
select ok(not (select must_change_password from public.profiles where id='14000000-0000-4000-8000-000000000008'),'purchasing flag becomes false');
select is((select count(*) from public.account_management_audit_logs where target_user_id='14000000-0000-4000-8000-000000000008' and action='password_changed'),1::bigint,'purchasing member gets one audit');
select ok((select must_change_password from public.profiles where id='14000000-0000-4000-8000-000000000009'),'inactive purchasing member remains forced to change password');
