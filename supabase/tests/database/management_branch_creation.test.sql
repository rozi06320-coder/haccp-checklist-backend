...
select is(has_function_privilege('authenticated','public.create_managed_branch(uuid,uuid,text,text,text,text,text,text,text,boolean)','execute'),false,'authenticated cannot execute branch creation RPC');
select is(has_function_privilege('service_role','public.create_managed_branch(uuid,uuid,text,text,text,text,text,text,text,boolean)','execute'),true,'service role can execute branch creation RPC');
select is(has_function_privilege('authenticated','public.create_internal_admin_branch(uuid,uuid,text,text,text,text,text,text,text,boolean)','execute'),false,'authenticated cannot execute internal admin branch creation RPC');
...
select is((select organization_id::text from internal_admin_created_branch),'2e000000-0000-4000-8000-000000000001','internal admin branch creation returns organization identity');
...
select is((select code from internal_admin_created_branch),'HUN-JED-017','internal admin branch code is manually supplied and uppercased');
select is((select city from internal_admin_created_branch),'Jeddah','internal admin branch creation returns normalized city');
select is((select area from internal_admin_created_branch),'Al Hamra','internal admin branch creation returns normalized area');
select is((select address from internal_admin_created_branch),'Corniche Road','internal admin branch creation returns trimmed address');
select is((select count(*) from public.branches where id = (select id from internal_admin_created_branch)),1::bigint,'internal admin branch row actually exists');
select is((select count(*) from public.branch_supervisor_teams where branch_id = (select id from public.branches where name='Admin Created Branch')),0::bigint,'internal admin branch creation does not create supervisor teams');
select is((select count(*) from public.operational_staff where branch_id = (select id from public.branches where name='Admin Created Branch')),0::bigint,'internal admin branch creation does not create operational staff');

create temporary table internal_admin_utc_branch on commit drop as
select *
from public.create_internal_admin_branch('1e000000-0000-4000-8000-000000000005','2e000000-0000-4000-8000-000000000001','Admin UTC Branch',null,'HUN-MKK-020','Makkah',null,null,'UTC',true);
...
select throws_ok($$select public.create_internal_admin_branch('1e000000-0000-4000-8000-000000000001','2e000000-0000-4000-8000-000000000001','Manager Through Admin RPC',null,'HUN-RUH-002','Riyadh',null,null,'Asia/Riyadh',true)$$,'42501','internal admin access required','manager cannot use internal admin branch creation RPC');
select throws_ok($$select public.create_internal_admin_branch('1e000000-0000-4000-8000-000000000003','2e000000-0000-4000-8000-000000000001','Supervisor Through Admin RPC',null,'HUN-RUH-002','Riyadh',null,null,'Asia/Riyadh',true)$$,'42501','internal admin access required','supervisor cannot use internal admin branch creation RPC');
...
select throws_ok($$select public.create_managed_branch('1e000000-0000-4000-8000-000000000002','2e000000-0000-4000-8000-000000000001','Cross Org',null,'HUN-RUH-002','Riyadh',null,null,'Asia/Riyadh',true)$$,'22023','invalid branch creation request','unrelated manager cannot create in another organization');
select throws_ok($$select public.create_managed_branch('1e000000-0000-4000-8000-000000000003','2e000000-0000-4000-8000-000000000001','Supervisor Branch',null,'HUN-RUH-002','Riyadh',null,null,'Asia/Riyadh',true)$$,'22023','invalid branch creation request','supervisor cannot create branch');
select throws_ok($$select public.create_managed_branch('1e000000-0000-4000-8000-000000000004','2e000000-0000-4000-8000-000000000001','Staff Branch',null,'HUN-RUH-002','Riyadh',null,null,'Asia/Riyadh',true)$$,'22023','invalid branch creation request','staff cannot create branch');
select throws_ok($$select public.create_managed_branch('1e000000-0000-4000-8000-000000000001','2e000000-0000-4000-8000-000000000001','   ',null,'HUN-RUH-002','Riyadh',null,null,'Asia/Riyadh',true)$$,'22023','invalid branch creation request','empty branch name is rejected');
