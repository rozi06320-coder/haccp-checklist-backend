...
select is((select count(*)::integer from public.branch_purchase_logs where source_type='central_purchasing'),2,'marking purchased creates one Branch Purchase Log row per request item');
