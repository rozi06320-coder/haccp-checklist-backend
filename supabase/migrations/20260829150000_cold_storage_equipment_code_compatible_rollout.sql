-- Backward-compatible Cold Storage equipment code rollout.
-- This migration supports old and new backend revisions during deployment:
-- old callers keep using the legacy name/type overloads, while new callers
-- can use equipment_code-aware overloads. Legacy overloads must be removed only
-- after production is proven to use the new contract.

alter table public.branch_cold_storage_equipment
  add column if not exists equipment_code text null;
...
begin
  if not exists (
    ...
    where constraint_row.conname = 'branch_cold_storage_equipment_code_check'
      and constraint_row.conrelid = 'public.branch_cold_storage_equipment'::regclass
  ) then
    alter table public.branch_cold_storage_equipment
  ...
  if not exists (
    select 1
    from pg_catalog.pg_constraint constraint_row
    where constraint_row.conname = 'cold_storage_equipment_code_check'
      and constraint_row.conrelid = 'public.cold_storage_equipment'::regclass
  ) then
    alter table public.cold_storage_equipment
...
create unique index if not exists branch_cold_storage_equipment_active_equipment_code_key
...
create or replace function private.clean_cold_storage_equipment_code(candidate text)
returns text
language plpgsql
immutable
security invoker
...
begin
  if not exists (
    ...
    where attribute.attrelid = 'public.supervisor_cold_storage_equipment_dto'::regclass
      and attribute.attname = 'equipment_code'
...
create or replace function public.create_supervisor_cold_storage_equipment(
  actor_user_id uuid,
  target_branch_id uuid,
  equipment_name text,
  equipment_type text
...
create or replace function public.update_supervisor_cold_storage_equipment(
  actor_user_id uuid,
  target_branch_id uuid,
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'items', coalesce((
      select pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
        'item_id', 'missed:' || slot,
        'item_text', slot || ' scheduled Chiller & Freezer check',
        'answer', 'not_checked',
...
revoke execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text) from public;
revoke execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text) from anon;
revoke execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text) from authenticated;
grant execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text) to service_role;

revoke execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text, text) from public;
revoke execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text, text) from anon;
revoke execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text, text) from authenticated;
grant execute on function public.update_supervisor_cold_storage_equipment(uuid, uuid, uuid, text, text, text) to service_role;
...
revoke execute on function public.archive_supervisor_cold_storage_equipment(uuid, uuid, uuid) from public;
revoke execute on function public.archive_supervisor_cold_storage_equipment(uuid, uuid, uuid) from anon;
revoke execute on function public.archive_supervisor_cold_storage_equipment(uuid, uuid, uuid) from authenticated;
grant execute on function public.archive_supervisor_cold_storage_equipment(uuid, uuid, uuid) to service_role;
