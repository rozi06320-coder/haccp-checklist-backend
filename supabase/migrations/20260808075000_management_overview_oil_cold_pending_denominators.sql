-- Keep missing Oil Tracking and due Refrigerator & Freezer checks in Manager Overview pending denominators.
...
begin
  ...
  active_branches as materialized (
    select branch.id, branch.name, branch.code, branch.timezone,
      ...
      extract(hour from snapshot_at at time zone branch.timezone) >= 3 sales_tracking_closed,
      case
  ...
  opening_metrics as materialized (
    select unit.branch_id, unit.team_id, unit.business_date, unit.checklist_type, unit.state,
      ...
      pg_catalog.count(result.id) filter (
        where unit.state = 'submitted' and result.answer in ('completed', 'issue_found')
      )::bigint answered_checks,
      pg_catalog.count(result.id) filter (where unit.state = 'submitted' and result.answer = 'completed')::bigint compliant_checks,
      pg_catalog.count(result.id) filter (where unit.state = 'submitted' and result.answer = 'issue_found')::bigint issue_checks
  ...
  hygiene_metrics as materialized (
    select unit.branch_id, unit.team_id, unit.business_date, unit.checklist_type, unit.state,
      ...
      pg_catalog.count(value.result) filter (where unit.state = 'submitted' and value.result in ('pass', 'issue'))::bigint answered_checks,
      pg_catalog.count(value.result) filter (where unit.state = 'submitted' and value.result = 'pass')::bigint compliant_checks,
      pg_catalog.count(value.result) filter (where unit.state = 'submitted' and value.result = 'issue')::bigint issue_checks
    from selected_units unit
  ...
  oil_submission_flags as materialized (
    select unit.branch_id, unit.team_id, unit.business_date, unit.state,
      unit.submission_id is null no_submission,
  ...
  oil_metrics as materialized (
    select flags.branch_id, flags.team_id, flags.business_date, 'oil_tracking'::text checklist_type, flags.state,
      case when flags.no_submission or flags.active_count > 0 then 2 else 0 end::bigint expected_checks,
  ...
  cold_metrics as materialized (
    select unit.branch_id, unit.team_id, unit.business_date, 'cold_storage'::text checklist_type, unit.state,
      pg_catalog.cardinality(unit.due_cold_slots)::bigint expected_checks,
  ...
  sales_tracking_unit_keys as materialized (
    select branch.id branch_id, team.team_id, branch.business_date, branch.sales_tracking_closed
    from active_branches branch
    join eligible_teams team on team.branch_id = branch.id
    union
    select report.branch_id, report.supervisor_team_id, report.business_date, branch.sales_tracking_closed
    from public.sales_tracking_reports report
    join active_branches branch
      on branch.id = report.branch_id
      and branch.business_date = report.business_date
    where report.organization_id = target_organization_id
      and report.state = 'submitted'
  ),
  sales_tracking_selected_units as materialized (
    select unit.branch_id, unit.team_id, unit.business_date, unit.sales_tracking_closed,
      report.id report_id, coalesce(report.state, 'not_started') state
    from sales_tracking_unit_keys unit
    left join lateral (
      select candidate.*
      from public.sales_tracking_reports candidate
      where candidate.organization_id = target_organization_id
        and candidate.branch_id = unit.branch_id
        and candidate.supervisor_team_id = unit.team_id
        and candidate.business_date = unit.business_date
  ...
  sales_tracking_report_variance as materialized (
    select unit.branch_id, unit.team_id, unit.business_date, unit.sales_tracking_closed, unit.state,
      coalesce(pg_catalog.sum(coalesce(row.actual_cash,0) + coalesce(row.actual_credit,0) + coalesce(row.online_delivery,0)
        - coalesce(row.pos_cash,0) - coalesce(row.pos_credit,0) - coalesce(row.online_delivery,0)),0) variance
    from sales_tracking_selected_units unit
    left join public.sales_tracking_sales_rows row on row.report_id = unit.report_id
    group by unit.branch_id, unit.team_id, unit.business_date, unit.sales_tracking_closed, unit.state
  ),
  sales_tracking_metrics as materialized (
    select row.branch_id, row.team_id, row.business_date, 'sales_tracking'::text checklist_type,
      case when row.state = 'submitted' then 'submitted' when row.sales_tracking_closed then 'not_started' else 'draft' end state,
      1::bigint expected_checks,
      case when row.state = 'submitted' then 1 else 0 end::bigint answered_checks,
      case when row.state = 'submitted' and pg_catalog.round(row.variance,2) = 0 then 1 else 0 end::bigint compliant_checks,
      case when row.state = 'submitted' and pg_catalog.round(row.variance,2) <> 0 then 1 else 0 end::bigint issue_checks
    from sales_tracking_report_variance row
  ),
  unit_metrics as materialized (
    ...
    union all
    select * from sales_tracking_metrics
  ),
