-- Forward-only migration: Update public.list_managed_people_directory to map
-- inactive staff with closure_reason = 'promoted_to_supervisor' to status 'promoted_to_supervisor'.

create or replace function public.list_managed_people_directory(
  actor_user_id uuid,
  ...
  people_rows as materialized (
    select
      ...
      pg_catalog.jsonb_build_object(
        ...
        'status', case
          when staff.employment_status = 'inactive'
            and latest_closed_assignment.closure_reason = 'promoted_to_supervisor'
            then 'promoted_to_supervisor'
          when staff.employment_status = 'inactive'
            then 'left_company'
          when duty.duty_status = 'on_vacation'
            then 'on_vacation'
          else 'active'
        end,
    ...
    left join lateral (
      select a.closure_reason
      from public.operational_staff_assignments a
      where a.operational_staff_id = staff.id
        and not a.active
      order by a.closed_at desc nulls last, a.valid_to desc nulls last, a.id desc
      limit 1
    ) latest_closed_assignment on staff.employment_status = 'inactive'
...
comment on function public.list_managed_people_directory(uuid, uuid, uuid, date, text, text) is
  'Additive Manager directory reader. Supervisors are returned once with all active branches in the authorized organization; branch_filter controls person inclusion. Promoted operational staff surface with status promoted_to_supervisor based on latest closed assignment closure_reason. Results are capped at 1000 with explicit total/truncation metadata.';
