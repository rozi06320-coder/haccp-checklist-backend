...
create or replace function private.branch_code_candidate(branch_name text, suffix int default 0)
returns text
language sql
immutable
security definer
...
as $$
  select case
    when suffix <= 0 then left(coalesce(nullif(pg_catalog.regexp_replace(pg_catalog.upper(pg_catalog.btrim(branch_name)), '[^A-Z0-9]+', '', 'g'), ''), 'BRANCH'), 12)
    else left(coalesce(nullif(pg_catalog.regexp_replace(pg_catalog.upper(pg_catalog.btrim(branch_name)), '[^A-Z0-9]+', '', 'g'), ''), 'BRANCH'), 9) || '-' || suffix::text
  end;
$$;

revoke all on function private.branch_code_candidate(text,int) from public, anon, authenticated;
