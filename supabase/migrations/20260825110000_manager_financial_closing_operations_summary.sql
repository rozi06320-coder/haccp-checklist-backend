...
as $$
  with active_branches as materialized (
    select branch.id,
      branch.timezone,
      (as_of at time zone branch.timezone)::date business_date,
    ...
    select branch.id,
      ...
      branch.local_clock >= time '02:00'
        and not exists (
          ...
          where report.organization_id = target_organization_id
            and report.branch_id = branch.id
            and report.business_date = branch.business_date - 1
            and report.state = 'submitted'
        ) overdue_prior_day
  ...
  select pg_catalog.jsonb_build_object(
    ...
    'overdue_prior_day', pg_catalog.count(*) filter (where status.overdue_prior_day)::integer,
