-- Promote an operational staff member into the same active team when that team is unsupervised.
...
declare
  ...
  existing_team_supervisor_row public.branch_operational_team_supervisors%rowtype;
...
begin
  ...
  if resolved_target_branch_id = staff_row.branch_id then
    perform 1
    from public.branch_operational_teams team
    where team.id = assignment_row.operational_team_id
    ...
    if exists (
      ...
      where supervisor_assignment.operational_team_id = assignment_row.operational_team_id
    ...
    select supervisor_assignment.* into existing_team_supervisor_row
    from public.branch_operational_team_supervisors supervisor_assignment
    where supervisor_assignment.operational_team_id = assignment_row.operational_team_id
      and supervisor_assignment.supervisor_user_id = new_supervisor_user_id
    order by supervisor_assignment.active desc, supervisor_assignment.updated_at desc, supervisor_assignment.created_at desc
    limit 1
    for update;

    if existing_team_supervisor_row.id is null then
      ...
      values(
        ...
        staff_row.branch_id,
        assignment_row.operational_team_id,
    ...
    elsif not existing_team_supervisor_row.active then
      update public.branch_operational_team_supervisors
      set assignment_role = 'primary',
          active = true,
          valid_from = current_date,
          valid_to = null
      where id = existing_team_supervisor_row.id;
    elsif existing_team_supervisor_row.assignment_role <> 'primary' then
      update public.branch_operational_team_supervisors
      set assignment_role = 'primary'
      where id = existing_team_supervisor_row.id;
    end if;
  ...
  values(
    ...
    pg_catalog.jsonb_build_object(
      ...
      'inherited_operational_team_id',
        case
          when resolved_target_branch_id = staff_row.branch_id then assignment_row.operational_team_id
          else null
        end
    )
  );
