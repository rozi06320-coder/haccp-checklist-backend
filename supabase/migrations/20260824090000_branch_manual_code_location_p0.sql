...
alter table public.branches
  drop constraint if exists branches_code_manual_format,
  add constraint branches_code_manual_format check (
    code = pg_catalog.upper(pg_catalog.btrim(code))
    and code ~ '^[A-Z0-9-]{1,120}$'
  ) not valid,
  drop constraint if exists branches_city_format,
  add constraint branches_city_format check (
    city is null
    or (
      city = pg_catalog.regexp_replace(pg_catalog.btrim(city), '\s+', ' ', 'g')
      and length(city) between 1 and 120
    )
  ) not valid,
  drop constraint if exists branches_area_format,
  add constraint branches_area_format check (
    area is null
    or (
      area = pg_catalog.regexp_replace(pg_catalog.btrim(area), '\s+', ' ', 'g')
      and length(area) between 1 and 120
    )
  ) not valid,
  drop constraint if exists branches_address_format,
  add constraint branches_address_format check (
    address is null
    or (
      address = pg_catalog.btrim(address)
      and length(address) between 1 and 240
    )
  ) not valid;
...
drop function if exists public.create_internal_admin_branch(uuid, uuid, text, text, boolean, text);
drop function if exists public.update_internal_admin_branch(uuid, uuid, uuid, text, text, text);
drop function if exists public.update_internal_admin_branch(uuid, uuid, uuid, text, text, text, text, text, text);
...
begin
  ...
  if exists (
    select 1 from public.branches branch
    where branch.organization_id = target_organization_id
      and branch.code = normalized_code
  ) then
...
begin
  ...
  if exists (select 1 from public.branches branch where branch.organization_id = p_organization_id and branch.code = normalized_code) then
    raise exception 'branch code already exists' using errcode = '23505';
  end if;
...
begin
  ...
  if exists (select 1 from public.branches branch where branch.organization_id = p_organization_id and branch.id <> p_branch_id and branch.code = normalized_code) then
    raise exception 'branch code already exists' using errcode = '23505';
  end if;
...
revoke all on function public.create_managed_branch(uuid, uuid, text, text, text, text, text, text, text, boolean) from public, anon, authenticated;
revoke all on function public.create_internal_admin_branch(uuid, uuid, text, text, text, text, text, text, text, boolean) from public, anon, authenticated;
revoke all on function public.update_internal_admin_branch(uuid, uuid, uuid, text, text, text, text, text, text, text) from public, anon, authenticated;

grant execute on function public.list_internal_admin_branches(uuid, uuid) to service_role;
grant execute on function public.create_managed_branch(uuid, uuid, text, text, text, text, text, text, text, boolean) to service_role;
grant execute on function public.create_internal_admin_branch(uuid, uuid, text, text, text, text, text, text, text, boolean) to service_role;
grant execute on function public.update_internal_admin_branch(uuid, uuid, uuid, text, text, text, text, text, text, text) to service_role;
