-- Avoid an ambiguous reference between the local business date and submission columns.
...
begin
  ...
  if exists(
    ...
    where snapshot.operational_staff_id=target_staff_id and submission.branch_id=target_branch_id
      and submission.business_date=current_business_date and submission.checklist_type='staff_hygiene' and submission.state='submitted'
  ...
  update public.operational_staff_assignments set active=false,valid_to=current_business_date where id=old_assignment.id;
  ...
  values(staff_row.organization_id,target_branch_id,target_staff_id,target_team.legacy_supervisor_team_id,
    target_team.id,old_assignment.operational_roles,current_business_date) returning id into created_assignment;
