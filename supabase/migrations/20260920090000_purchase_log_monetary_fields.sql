...
alter table public.branch_purchase_logs
  add constraint branch_purchase_logs_before_tax_amount_money_check
  check (before_tax_amount is null or before_tax_amount >= 0),
  add constraint branch_purchase_logs_tax_amount_money_check
  check (tax_amount is null or tax_amount >= 0),
  add constraint branch_purchase_logs_monetary_breakdown_pair_check
  check (
    (before_tax_amount is null and tax_amount is null)
    or (before_tax_amount is not null and tax_amount is not null)
  ),
  add constraint branch_purchase_logs_monetary_amount_check
  check (before_tax_amount is null or amount = before_tax_amount + tax_amount);
...
begin
  ...
  select l.id,l.branch_id,b.name,l.category,l.item_name,l.quantity,l.amount,
    ...
    l.invoice_original_name,l.created_by,l.created_at,l.updated_at
...
begin
  ...
  return query insert into public.branch_purchase_logs(
    ...
    reimbursement_note,reimbursed_at,reimbursed_by,invoice_storage_path,invoice_original_name,created_by
  )
  values(
